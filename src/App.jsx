...
export default function SnackCheck() {
  ...
  const shareModal=shareRating&&(()=>{
    ...
    return (
      <div style={{position:"fixed",inset:0,background:"#000",zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}} onClick={()=>setShareRating(null)}>
        <div onClick={e=>e.stopPropagation()} style={{width:280,borderRadius:20,overflow:"hidden",display:"flex",flexDirection:"column",background:"#1a1a1a",flexShrink:0}}>
          {hasImg
            ?<div style={{height:190,position:"relative",flexShrink:0}}><img src={r.image} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,#1a1a1a 100%)"}}/></div>
            :<div style={{height:150,background:`linear-gradient(135deg,${P.orange} 0%,${P.orangeDark} 100%)`,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",flexShrink:0}}><span style={{fontSize:60}}>{CAT_ICONS[catIdx]||"🍿"}</span><div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 40%,#1a1a1a 100%)"}}/></div>
          }
          <div style={{padding:"14px 18px 18px"}}>
            <div style={{fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:6}}>MY SNACK VERDICT</div>
            <div style={{fontSize:11,fontWeight:700,color:P.orange,textTransform:"uppercase",letterSpacing:1,marginBottom:2}}>{r.brand}</div>
            <div style={{fontSize:17,fontWeight:800,color:"white",lineHeight:1.2,marginBottom:6}}>{dn}{r.flavor?` — ${r.flavor}`:""}</div>
            <div style={{fontSize:22,color:P.orange,letterSpacing:3,marginBottom:10}}>{"★".repeat(r.score)}{"☆".repeat(5-r.score)}</div>
            {(r.pros?.length>0||r.cons?.length>0)&&(
              <div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:14}}>
                {r.pros?.slice(0,2).map((p,i)=><span key={i} style={{background:"rgba(245,166,35,0.18)",color:P.orange,borderRadius:20,padding:"4px 10px",fontSize:11,fontWeight:600}}>✓ {p}</span>)}
                {r.cons?.slice(0,1).map((c,i)=><span key={i} style={{background:"rgba(255,255,255,0.08)",color:"rgba(255,255,255,0.65)",borderRadius:20,padding:"4px 10px",fontSize:11}}>✗ {c}</span>)}
              </div>
            )}
            <div style={{fontSize:9,color:"rgba(255,255,255,0.22)",letterSpacing:"0.08em",textTransform:"uppercase"}}>snackscheck.com · Rate snacks. Find the good stuff.</div>
          </div>
        </div>
        <div style={{marginTop:20,fontSize:11,color:"rgba(255,255,255,0.15)"}}>tap anywhere to close</div>
      </div>
    );
  })();
