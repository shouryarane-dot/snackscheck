...
const LANGS = {
  ...
  fr: { appSub:"Outil de notation", allSnacks:"Tous les snacks", myRatings:"Mes notes", search:"Rechercher...", addRating:"Noter", close:"✕ Fermer", save:"Enregistrer ✓", saved:"Enregistré!", savedSub:"Votre note a été ajoutée.", back:"← Retour", reviews:"Avis", rateFirst:"Noter votre premier snack →", noRatings:"Aucun snack noté", noRatingsSub:"Ajoutez le premier via le bouton +", noMyRatings:"Aucune note de", welcomed:"Bienvenue sur SnackCheck", welcomeSub:"Comment vous appelle-t-on?", yourName:"Votre nom", namePh:"ex. Alex", start:"Commencer →", brand:"Marque", brandPh:"ex. Lays, Haribo", product:"Nom du produit", productPh:"ex. Ovenbaked, Classic", flavor:"Saveur / Variante", flavorPh:"ex. Paprika, Sel de mer", category:"Catégorie", rating:"Note", pros:"Points positifs", prosPh:"ex. Croustillant, Bien salé", cons:"Points négatifs", consPh:"ex. Trop gras, Sans goût", photo:"Photo", photoOptional:"(optionnel)", photoTap:"Appuyez pour ajouter une photo", minScore:"Score min.", onlyMulti:"Seulement 2+ notes", reset:"✕ Réinitialiser", comma:"(séparé par virgule)", required:"*", scoreLabels:["","😕 Mauvais","😐 Moyen","🙂 OK","😋 Bon","🤩 Excellent!"], rated:"Noté", avgScore:"Score moy.", categories:"Catégories", ratingsCount:(n)=>`${n} avis`, products:(n)=>`${n} produit${n!==1?"s":""}`, myRatingsCount:(n,name)=>`${n} avis de ${name}`, you:" (moi)", sorts:["Plus récent","Plus ancien","Meilleur score","Score le plus bas","Plus noté","A → Z"], cats:["Tout","Chips","Biscuits","Noix","Bonbons","Choco","Puffs","Barres","Autre"],
    landing:{ hero1:"Notez vos snacks.", hero2:"Trouvez les meilleurs.", sub:"Une base de données de snacks par la communauté. Voyez ce que les autres pensent avant d'acheter — ou partagez votre avis.", cta1:"Commencer à noter", cta2:"Parcourir la base", how:"Comment ça marche", howTitle:"Des avis honnêtes de vrais amateurs de snacks", howSub:"Pas de reviews sponsorisées. Pas d'algorithmes. Juste des gens qui notent ce qu'ils ont vraiment mangé.", f1t:"Notez n'importe quel snack", f1d:"Score de 1 à 5 étoiles, ajoutez des points positifs et négatifs, uploadez une photo.", f2t:"Base de données partagée", f2d:"Tous les avis au même endroit — tout le monde peut consulter et contribuer.", f3t:"Nutri-Score & filtres allergènes", f3d:"Notes Nutri-Score, filtres allergènes et données nutritionnelles — sur 10 000+ produits.", example:"Exemple d'avis", exampleTitle:"Voilà à quoi ressemble un avis", join:"Rejoindre gratuitement", joinTitle:"Rejoignez la communauté — c'est gratuit", joinSub:"Créez un compte pour ajouter vos avis et construire votre historique de snacks.", joinBtn:"Créer un compte gratuit", footer:"snackscheck.com · Fait avec amour pour les amateurs de snacks", privacy:"Politique de confidentialité", exPro1:"Croustillant", exPro2:"Bien assaisonné", exCon1:"Cher", f4t:"Gagnez des badges et grimpez le classement", f4d:"Notez des snacks pour gagner des points et progresser de Snack Scout à Snack Connoisseur. Les meilleurs reviewers peuvent débloquer des récompenses exclusives — comme des snacks gratuits avant leur lancement. Suivez @snacks_check sur Instagram pour ne rien rater.", badgesLabel:"Niveaux de badges", badgesTitle:"Montez de Scout à Connaisseur", badgesSub:"Gagnez des points à chaque note. Les meilleurs reviewers gagnent des snacks exclusifs avant le lancement — suivez @snacks_check sur Instagram." },
...
export default function SnackCheck() {
  ...
  const shareModal=shareRating&&(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.88)",zIndex:300,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20}} onClick={e=>e.target===e.currentTarget&&setShareRating(null)}>
      {(()=>{
        const r=shareRating;
        ...
        return (
          <div style={{width:300,minHeight:300,background:P.orange,borderRadius:20,overflow:"hidden",display:"flex",flexDirection:"column",flexShrink:0}}>
            {hasImg&&<div style={{height:140,position:"relative",flexShrink:0}}><img src={r.image} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/><div style={{position:"absolute",inset:0,background:"linear-gradient(to bottom,transparent 50%,rgba(245,166,35,1) 100%)"}}/></div>}
            <div style={{flex:1,padding:"16px 18px 18px",display:"flex",flexDirection:"column",justifyContent:"space-between"}}>
              {!hasImg&&<div style={{fontSize:42,textAlign:"center",marginBottom:4}}>{CAT_ICONS[catIdx]||"🍿"}</div>}
              <div>
                <div style={{fontSize:9,color:"rgba(255,255,255,0.65)",letterSpacing:"0.12em",marginBottom:3}}>MY SNACK VERDICT</div>
                <div style={{fontSize:17,fontWeight:800,color:"white",lineHeight:1.2,fontFamily:"system-ui,-apple-system,sans-serif"}}>{r.brand} {dn}</div>
                {r.flavor&&<div style={{fontSize:11,color:"rgba(255,255,255,0.78)",marginTop:2}}>{r.flavor}</div>}
                <div style={{fontSize:20,color:"white",marginTop:6,letterSpacing:2}}>{"★".repeat(r.score)}{"☆".repeat(5-r.score)}</div>
              </div>
              {(r.pros?.length>0||r.cons?.length>0)&&(
                <div style={{display:"flex",flexWrap:"wrap",gap:4,margin:"10px 0 4px"}}>
                  {r.pros?.slice(0,2).map((p,i)=><span key={i} style={{background:"rgba(255,255,255,0.25)",color:"white",borderRadius:20,padding:"3px 9px",fontSize:10,fontWeight:700}}>✓ {p}</span>)}
                  {r.cons?.slice(0,1).map((c,i)=><span key={i} style={{background:"rgba(0,0,0,0.18)",color:"rgba(255,255,255,0.9)",borderRadius:20,padding:"3px 9px",fontSize:10}}>✗ {c}</span>)}
                </div>
              )}
              <div style={{fontSize:9,color:"rgba(255,255,255,0.55)",letterSpacing:"0.08em",textTransform:"uppercase",marginTop:8}}>snackscheck.com · Rate snacks. Find the good stuff.</div>
            </div>
          </div>
        );
      })()}
      <p style={{color:"white",fontSize:15,fontWeight:700,marginTop:18,textAlign:"center"}}>📸 Screenshot & share!</p>
      <p style={{color:"rgba(255,255,255,0.5)",fontSize:12,marginTop:4,textAlign:"center"}}>Show the world your snack verdict</p>
      <button onClick={()=>setShareRating(null)} style={{marginTop:14,background:"rgba(255,255,255,0.12)",color:"white",border:"1.5px solid rgba(255,255,255,0.25)",borderRadius:12,padding:"9px 28px",fontWeight:700,fontSize:14,cursor:"pointer"}}>✕ Close</button>
    </div>
  );
              ...
              return (
                <div key={r.id} onClick={()=>{setSelProd(r.productCode);setView("detail");}}
                  ...
                  <div style={{textAlign:"right",flexShrink:0,display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4}}>
                    ...
                    <button onClick={e=>{e.stopPropagation();setShareRating(r);}} style={{background:P.orangeLight,color:P.orange,border:`1px solid ${P.orange}`,borderRadius:8,padding:"3px 9px",fontSize:10,fontWeight:700,cursor:"pointer"}}>Share ↗</button>
                  </div>
                </div>
              );
            })}
